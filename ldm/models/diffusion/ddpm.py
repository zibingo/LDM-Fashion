...
class DDPM(pl.LightningModule):
    ...
    def get_input(self, batch, k,k1,k2):
        #---------------------------------------------------
        x,s,t = batch[k],batch[k1],batch[k2]
        if len(x.shape) == 3:
            x = x[..., None]
        if len(s.shape) == 3:
            s = s[..., None]
        ...
        s = s.to(memory_format=torch.contiguous_format).float()
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.is_use_style_loss = is_use_style_loss
        #--------------------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------------------
        self.instantiate_vgg_loss(vgg_loss_config)
        # ------------------------------------------------------------------------------
    ...
    def instantiate_vgg_loss(self,config):
        self.vgg_loss = instantiate_from_config(config)
    ...
    def get_learned_conditioning(self, c):
        sketch = c["sketch"]
        texture = c["texture"]
        c = self.cond_stage_model(sketch,texture)
        return c
    ...
    @torch.no_grad()
    def get_input(self, batch,first_stage_key,cond_stage_key_1,cond_stage_key_2,
                  return_first_stage_outputs=False, force_c_encode=False,
                  cond_key=None, return_original_cond=False, bs=None):
        x,sketch,texture = super().get_input(batch, first_stage_key,cond_stage_key_1,cond_stage_key_2)
        if bs is not None:
            x = x[:bs]
        x = x.to(self.device)
        ...
        if random_number <  self.probability_of_discard:
            ...
            c = {"sketch": sketch, "texture": texture}
        #---------------------------------------------------------------------------------------------------------
        out = [z, c]
    ...
    def shared_step(self, batch, **kwargs):
        x, c = self.get_input(batch, self.first_stage_key,self.cond_stage_key_1,self.cond_stage_key_2)
        loss = self(x, c)
        return loss
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.cond_stage_trainable:
            c = self.get_learned_conditioning(c)
        condition = {"cond":c}
        return self.p_losses(x, condition, t, *args, **kwargs)
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        # 默认l_simple_weight=1.0,original_elbo_weight=0.0
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        log = dict()
        z, c, x, xrec = self.get_input(batch, 
                                           self.first_stage_key,
                                           self.cond_stage_key_1,
                                           self.cond_stage_key_2,
